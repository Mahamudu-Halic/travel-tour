...
export default async function DashboardPage() {
  ...
  const { data: bookings, count: totalBookings } = await supabase
    .from("bookings")
    .select("*, tours(title, slug, destination, image_url, category)", { count: "exact" })
    .eq("user_id", user.id)
  ...
  return (
      ...
      <div className="flex-1 bg-muted/30 py-12 md:py-16">
        <div className="container mx-auto max-w-7xl px-4">
          <div className="mb-8">
            <h1 className="mb-2 text-3xl font-bold text-balance md:text-4xl font-serif">
              Welcome back, {profile?.full_name || "Traveler"}!
            </h1>
            <p className="text-muted-foreground">Manage your bookings and explore new adventures</p>
          </div>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
                  ...
                  {bookings && bookings.length > 0 ? (
                    <div className="space-y-4">
                      {bookings.map((booking) => (
                        <div key={booking.id} className="flex items-start gap-4 rounded-lg border border-border/40 p-4">
            ...
            <div className="lg:col-span-1">
              ...
              <Card className="mt-6 border-border/50 bg-primary text-primary-foreground">
                <CardContent className="p-6">
                  <h3 className="mb-2 text-lg font-semibold font-serif">Explore More</h3>
